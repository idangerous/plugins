...
$.fn.moover = function(a) {
	...
	var mooverTransforms = {
		'typewriter' : {
			'default' : {
				initCharTransform : "scale(0.01,0.01) rotate(0deg)",
				middleCharTransform : "scale(1.2,1.3) rotate(5deg)",
				finalCharTransform : "scale(1,1) rotate(0deg)",
				charOrigin : 'center bottom'
			},
			'rotators' : {
				initCharTransform : "scale(0.01,0.01) rotate(-180deg)",
				middleCharTransform : "scale(0.8,0.8) rotate(180deg)",
				finalCharTransform : "scale(1,1) rotate(0deg)",
				charOrigin : 'center center'
			},
			'fall' : {
				initCharTransform : "scale(1,1) translate3d(0,-1000px,0)",
				middleCharTransform : "scale(1,1) translate3d(0,0,0px)" ,
				finalCharTransform : "scale(1,1) translate3d(0,0,0px)",
				charOrigin : 'center bottom'
			},
			'fall-elastic' : {
				initCharTransform : "scale(1,1) translate3d(0,-1000px,0)",
				middleCharTransform : "scale(1,0.5) translate3d(0,0,0px)" ,
				finalCharTransform : "scale(1,1) translate3d(0,0,0px)",
				charOrigin : 'center bottom'
			},
			'fall-reverse' : {
				initCharTransform : "scale(1,1) translate3d(0,-1000px,0)",
				middleCharTransform : "scale(1,-0.5) translate3d(0,0,0px)" ,
				finalCharTransform : "scale(1,1) translate3d(0,0,0)",
				charOrigin : 'center bottom'
			},
			'flippers-v' : {
				initCharTransform : "scale(0.01,0.01)",
			...
			'rain-elastic' : {
				initCharTransform : "scale(1,30) rotate(80deg) translate3d(0px, -500px,0)",
				middleCharTransform : "scale(1,-0.8) rotate(10deg) translate3d(0px, 0px,0)",
				finalCharTransform : "scale(1,1) rotate(0deg) translate3d(0px, 0px,0)",
				charOrigin : 'center bottom'
			},
			'hurricane' : {
				initCharTransform : "scale(0.01,0.01) rotate(0deg) translate3d(500px, 0px, 0)",
				middleCharTransform : "scale(1,1) rotate(30deg) translate3d(-500px, 0px, 0)",
				finalCharTransform : "scale(1,1) rotate(0deg) translate3d(0px, 0px, 0)",
				charOrigin : 'center center'
			}
		},
		'slide': {
			...
			'skew' : {
				initTextTransform: "skewX(-40deg) scale(1,0.01)",
				middleTextTransform : "skewX(0deg) scale(1,1)",
				finalTextTransform: "skewX(-40deg) scale(1,0.01)"
			},
			'flippers-h' : {
				initTextTransform: "scale(-1,0.01)",
				middleTextTransform : "scale(1,1)",
				finalTextTransform: "scale(-1,0.01)"
			},
			'flippers-v' : {
				initTextTransform: "scale(0.01,-1)",
				middleTextTransform : "scale(1,1)",
				finalTextTransform: "scale(0.01,-1)"
			},
			'scale-full' : {
				initTextTransform: "scale(0.01,0.01)",
				middleTextTransform : "scale(1,1)",
				finalTextTransform: "scale(0.01,0.01)"
			},
			'scale-y' : {
				initTextTransform: "scale(1,0.01)",
				middleTextTransform : "scale(1,1)",
			...
			'rotation' : {
				initTextTransform: "scale(0.01,0.01) rotate(180deg)",
				middleTextTransform : "scale(1,1) rotate(0deg)",
				finalTextTransform: "scale(0.01,0.01) rotate(-180deg)"
			},
			'hurricane' : {
				initTextTransform: "scale(0.01,0.01) rotate(360deg) translate3d(-200px,500px,0)",
				middleTextTransform : "scale(1,1) rotate(0deg) translate3d(0px, 0px,0)",
				finalTextTransform: "scale(0.01,0.01) rotate(-360deg) translate3d(-200px,500px,0)"
			}
		},
		'timings' : {
			'very-fast' : {
	...
	a.initTextTransform = a.initTextTransform || "scale(1,0.01) skewX(40deg)"
	a.middleTextTransform = a.middleTextTransform || "scale(1,1) skewX(0deg)"
	a.finalTextTransform = a.finalTextTransform || "scale(1,0.01) skewX(-40deg)"
	...
	a.initCharTransform = a.initCharTransform || "scale(0.01,0.01) rotate(0deg)"
	...
	;(function(){
		...
		mv.has3d = function(){ return has3d }
	...
	$.fn.mvTransform = function(params,callback) {
			...
			return this.each(function(){
				var es = $(this)[0].style
				if (params.transform) {
					if (!mv.has3d() && params.transform.indexOf('translate3d')>=0) {
					...
					if (mv.has3d() && params.transform.indexOf('translate')<0) {
	...
	/* Init HTML */
	mv.html = container.html();
	mv.htmlOuter = container[0].outerHTML;
	...
	function moove(f, direction, index) {		
		...
		if (activeSlide.length>0) {
			...
			if (f.stopOnLast) {
				if(nextSlide.index()==	container.find('.moover-slide').length-1) {
					if (f.stopButton) $(f.stopButton).click();
					mv.stop();
				}
			...
			textBlock.children("p").each(function(){
				...
				if (isAbsolute) {
					...
					if (text.css('left').indexOf('%')>=0) {
						...
						if(isNaN(text.finalLeft)) text.finalLeft = 0;
					}
						...
						text.mvTransitionEnd(function(){
							...
							setTimeout(function(){
								text.mvTransitionEnd(function(){
									setTimeout(function(){
										if (f.transformPreset) {
											f.finalTextTransform = mooverTransforms['slide'][f.transformPreset].finalTextTransform
										}
			...
			if (!isAbsolute) {
				...
				textBlock.children("p").each(function(){
					...
					if ($(this).css('left').indexOf('%')>=0) {
						...
						if(isNaN(pushLeftPos)) pushLeftPos = 0;
					}
					else {
			...
			function fadeLines() {
				textBlock.children("p").each(function(){
					...
					if(withCSS3()) {
						...
						tX = f.fadeType=='horizontal' ? (isEven ? -20 : 20) : 0;
						tY = f.fadeType=='horizontal' ?  0 : (isEven ? -20 : 20);
						el.css({left: leftPos+tX, top: topPos+tY}).delay(index*f.fadeDelay).animate({left:leftPos, top:topPos, opacity:1}, f.fadeTime*1, function(){
							
							
							if (index==totalLines-1) {
								var mvQueue = function(){
									textBlock.children("p")
									.animate({left: '+='+tX, top: '+='+tY, opacity:0}, f.slideTime, function(){
										removePrevSlide();
									})
									activateNextSlide();
								}
	...
	if (a.reloadOnTabSwitch) {
		mv.reload = function(){
			var newC = mv.container.clone().insertAfter(mv.container).html(mv.html);
			if (newC.parents().lengh==0) return;
			mv.container.remove();
			if ( a.prevButton && $(a.prevButton).length>0 ) {
				var prev = $(a.prevButton);
				prev.die().unbind().undelegate();
				prev.clone().insertAfter( $(a.prevButton) );
				prev.remove();
			}
			if ( a.nextButton && $(a.nextButton).length>0 ) {
				var next = $(a.nextButton);
				next.die().unbind().undelegate();
				next.clone().insertAfter( $(a.nextButton) );
				next.remove();
			}
			if ( a.playButton && $(a.playButton).length>0 ) {
				var play = $(a.playButton);
				play.die().unbind().undelegate();
				play.clone().insertAfter( $(a.playButton) );
				play.remove();
			}
			if ( a.stopButton && $(a.stopButton).length>0 ) {
				var stopButton = $(a.stopButton);
				stopButton.die().unbind().undelegate();
				stopButton.clone().insertAfter( $(a.stopButton) );
				stopButton.remove();
			}
			//mv = newC.moover(a);
			$.extend(mv, newC.moover(a));
		};
		mv.visibilityChange = function(func){
			...
			function onchange (evt) {
				...
				if (state=='hidden') mv.isVisible = false;			
				if (state=='visible') {
					mv.reload();
